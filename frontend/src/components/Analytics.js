...
const Analytics = () => {
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      const cacheKey = `analytics_${selectedPlatform}_${timeRange}`;
      ...
      const baseUrl = selectedPlatform === 'facebook' 
        ? 'http://localhost:8000/facebook'
        : 'http://localhost:8000/instagram';
      
      const dashboardUrl = selectedPlatform === 'facebook'
      ...
      const [dashboardRes, performanceRes, audienceRes, engagementTimeRes, trendsRes, reachRes] = await Promise.all([
        fetch(dashboardUrl),
        fetch(`${baseUrl}/insights/performance`).catch(() => ({ ok: false })),
        fetch(`${baseUrl}/insights/audience`).catch(() => ({ ok: false })),
        fetch(`${baseUrl}/insights/engagement-time`).catch(() => ({ ok: false })),
        fetch(`${baseUrl}/insights/engagement-trends`).catch(() => ({ ok: false })),
        fetch(`${baseUrl}/insights/reach`).catch(() => ({ ok: false }))
      ]);
      
      const [dashboard, performance, audience, engagementTime, trends, reach] = await Promise.all([
        dashboardRes.json(),
        performanceRes.ok ? performanceRes.json() : { success: false },
        audienceRes.ok ? audienceRes.json() : { success: false },
        engagementTimeRes.ok ? engagementTimeRes.json() : { success: false },
        trendsRes.ok ? trendsRes.json() : { success: false },
        reachRes.ok ? reachRes.json() : { success: false }
      ]);
      ...
      setError(`Failed to load ${selectedPlatform === 'facebook' ? 'Facebook' : 'Instagram'} analytics data`);
    } finally {
      setLoading(false);
    }
  };
